/*
 *  SoftLight.frag, based on Multiply.frag in Hexels 3
 *  Hexels 2
 *
...
void main()
{
        ...
        vec3 lighter = 1.0 - (1.0 - otex.rgb) * (1.0 - (incoming.rgb - 0.5));
        vec3 darker = otex.rgb * (incoming.rgb + 0.5);

        vec3 malted = 1.0 - (1.0 - otex.rgb) / incoming.rgb;
        if (incoming.rgb == vec3(0.0, 0.0, 0.0)) {
          malted = vec3(0.0, 0.0, 0.0);
        }
