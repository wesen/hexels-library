...
UI_Uniform_Float(Vibrance, 0., 8., 1);
UI_Uniform_Float(contrast, -2., 2., 1.2);
UI_Uniform_Float(brightness, 0., 1., 0.15);
UI_Uniform_Float(saturation, 0., 2., 1.5);
UI_Uniform_Float(hue, 0., 1., 0.);
...
*/
vec4 brightnessAdjust( in vec4 color, in float b) {
  return vec4(color.rgb + b, color.a);
}
...
vec4 contrastAdjust( in vec4 color, in float c) {
    float t = 0.5 - c * 0.5;
    return vec4(color.rgb * c + t, color.a);
}
...
{
    // uniforms
    float _vibrance = Vibrance;
    float _hue = hue;
    
	vec4 incoming = texture2D(_Layer, gl_TexCoord[0].xy);
    vec4 color = incoming;
    
    vec4 fragColor = saturationMatrix(saturation) * color; 
    fragColor = brightnessAdjust(fragColor, brightness); 
    fragColor = contrastAdjust(fragColor, contrast); 
